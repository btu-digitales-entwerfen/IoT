    ...
    public class MQTTreadV7 : GH_Component
    {
        ...
        async void Subscribe_Topic()
        {
            try
            {
                ...
                var mqttClientOptions = new MqttClientOptionsBuilder().WithTcpServer(uri.Host).Build();


                _Client.ConnectedAsync += async e =>
                {
                    Console.WriteLine("Connected successfully with MQTT Brokers.");
                    await _Client.SubscribeAsync(new MqttTopicFilterBuilder().WithTopic(topic).WithQualityOfServiceLevel((MqttQualityOfServiceLevel)qos).Build());
                };
            ...
            {
                Debug.WriteLine(e);
                AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "Can't connect to broker - check connection or address");
            }
        }
